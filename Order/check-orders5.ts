...
async function checkOrders() {
    ...
    const sOrders = (sData.orders || []).filter((o: any) => JSON.stringify(o).includes('***REDACTED***'));
    console.log(JSON.stringify(sOrders.map((o: any) => ({ status: o.status, created: o.createdAt, id: o.id })), null, 2));

    const sInvoices = (sData.invoices || []).filter((o: any) => JSON.stringify(o).includes('***REDACTED***'));
