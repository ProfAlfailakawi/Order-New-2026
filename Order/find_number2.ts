...
async function check() {
  ...
  const index = cleanStr.indexOf('***REDACTED***');
  if (index > -1) {
    console.log("FOUND!");
    // find where
    const orders = data.orders || [];
    for(const o of orders) {
      if ((o.customerPhone||"").replace(/\D/g, "").includes("***REDACTED***") || (o.phone||"").replace(/\D/g, "").includes("***REDACTED***")) {
        console.log("Found in orders:", o.customerName);
      }
    }
    const inv = data.invoices || [];
    for(const i of inv) {
      if ((i.customerPhone||"").replace(/\D/g, "").includes("***REDACTED***") || (i.phone||"").replace(/\D/g, "").includes("***REDACTED***")) {
        console.log("Found in invoices:", i.customerName);
      }
    }
    const cust = data.customers || [];
    for(const c of cust) {
      if ((c.phone||"").replace(/\D/g, "").includes("***REDACTED***")) {
